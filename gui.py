...
class App(QMainWindow):
    ...
    def append_data(self, data):
        ...
        if data is None:
            print(self.mydata)
            print(
                "Finished capturing (%d, %d)" % (len(self.mydata), len(self.mydata[0]))
            )
            print(self.mydata)
            self.updatePlots()
    ...
    def doSave(self):
        ...
        fname, ftype = QFileDialog.getSaveFileName(
            self, "Save File", self.storeFName, fmt[0]
        )
    ...
    def set_capture_size(self):
        self.capture_size = int(
            ceil(self.captureTime_s * 1000 / self.sampleIntervall_ms)
        )
        self.set_status("Capture size %d" % self.capture_size)
        print(
            "Capture size is now %d: captureTime %d s, sampleInterval %d ms"
            % (self.capture_size, self.captureTime_s, self.sampleIntervall_ms)
        )
