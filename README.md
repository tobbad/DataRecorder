...
Die aktuellen Daten der zwei erfassten Kanäle werden als *data1* und *data2* als einem NumPy Array Property bereitgestellt. Vom DataRecorder wird auf dem passenden *DataSet* eine *append* Funktion mit der Erfassungsdaten aufgeruft die je nach Name der im Konstruktor übergeben wurde mit dem übergebenen Closure gespeichert. So wird bei einer laufenden Erfassung im not-connected-State die Daten in eine Datei mit mit dem Zeitstempel die "nan" einhält ()   
